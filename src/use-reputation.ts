 ...
 */
export function useReputation(address?: string): UseReputationReturn {
  ...
  const fetchReputation = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/v1/reputation/${encodeURIComponent(resolvedAddress)}`,
        {
          method: 'GET',
          headers: {
      ...
      const data: ReputationInfo = await response.json();
